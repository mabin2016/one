...
class BackModel extends Model {
	...
	public function getSearch( $map = array(), $page = 1 ) {
		...
		$field = array('id','c_chemicals','c_name_one','c_name_sec','c_cas','c_img');
		$data = M()->table(C('DB_PREFIX').'compound' )->field( $field )->where( $map )
			->limit( $limit )
//									    ->order( "a.wb_id desc" )
			->select();
		$return['data'] = $data;
	...
	public function reactionList( $map ) {
		$data = M()->table(C('DB_PREFIX').'compound_relation' )->where( $map )->select();
		return $data;
	}
	 ...
	 * @return array
	 */
	public function getReaction( $marks ) {
//		$data = M()->table(C('DB_PREFIX').'compound_relation' )->where( $marks )->select();
		$data = M()->table(C('DB_PREFIX').'compound_relation' )->select();
//		p($data);die;
		$data = 11;
		return $data;
	}
